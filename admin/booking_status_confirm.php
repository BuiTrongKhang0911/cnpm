...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirmId'])) {
    $confirmBooking = $_POST['confirmId'];

    $stmt = mysqli_prepare($conn, "UPDATE booking SET Status='Confirm' WHERE BookingNumber=?");
    mysqli_stmt_bind_param($stmt, "i", $confirmBooking);
...
$sql = "SELECT * FROM booking where Status like 'Confirm'";
