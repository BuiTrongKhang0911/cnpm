<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($success){
            $sql = "UPDATE brands SET brandName=? WHERE Id=?";

            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt,"si",$brandname,$id);

            if (mysqli_stmt_execute($stmt)) {
                header("Location: manage_brands.php");
                exit;
            } else {
                echo "Error updating brand: " . mysqli_stmt_error($stmt);
            }
            mysqli_stmt_close($stmt);
        }
    }
...
<head>
    ...
    <title>Car Rental Portal | Admin Update Vehicle</title>
    <style>
        .form-control {
                                    ...
                                    <div class="panel-body">
                                        <form method="POST" action="" class="form-horizontal" enctype="multipart/form-data">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-8">
                                                    <input type="text" class="form-control" name="brandname" required value="<?php echo htmlspecialchars($brand['brandName'] ?? ''); ?>">
                                                </div>
                                            </div>
